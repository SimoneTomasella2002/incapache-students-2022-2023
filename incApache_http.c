									 ...
									 char *filename, struct stat *stat_p)
{
	...
	case RESPONSE_CODE_NOT_FOUND:
		...
		if ((fd = open(HTML_404, O_RDONLY)) >= 0)
		{
			...
			// Si ottiene il mime_type del file HTML_404 (Protocollo MIME, usato come estensione dei 
			// servizi mail per la codifica di dati diversi dal testo)
			mime_type = get_mime_type(HTML_404);
				
			// Se stat_p (metadati del file) è vuoto...
			if(stat_p == NULL){
				// ... azzeriamo stat_p ...
				stat_p = &stat_buffer;
					
				// ... e recuperiamo i metadati di HTML_404
				if(stat(HTML_404, stat_p) == -1)
					fail_errno("error in stat HTML_404");
			}
			
			// Recuperiamo la dimensione del file e la data dell'ultima modifica
			file_size = stat_p->st_size;
			file_modification_time = stat_p->st_mtime;
	...
	default:
		...
		if ((fd = open(HTML_501, O_RDONLY)) >= 0)
		{
			...
			// Stessa cosa di HTML_404, ma applicata a HTML 501
...
#ifdef INCaPACHE_6_1
	strcat(http_header, "\r\nServer: incApache 6.1 for SETI.\r\n");
	if (response_code >= 500 || is_http1_0)
		strcat(http_header, "Connection: close\r\n");
#else
	strcat(http_header, "\r\nServer: incApache 6.0 for SETI.\r\n");
	strcat(http_header, "Connection: close\r\n");
#endif
	...
	if (fd >= 0)
	{
		...
		if(header_sent = sendfile(client_fd, fd, NULL, file_size) == -1)
			fail_errno("sendfile error in http");

		if(close(fd) == -1)
			fail_errno("close fd error in http");
...
#define MethodIsConditional(m) ((m)&METHOD_CONDITIONAL)
	...
	while (getline(&http_request_line, &n, client_stream) >= 0)
	{
		...
		// In dubbio, non sono sicuro del controllo di errori

		method_str = strtok_r(http_request_line, " ", &strtokr_save);
		filename = strtok_r(NULL, " ", &strtokr_save);
		protocol = strtok_r(NULL, "\r", &strtokr_save);
				 ...
				 free(http_option_line), http_option_line = NULL, n = 0)
		{
			...
			if (option_name != NULL)
			{
				if (strcmp(option_name, "Cookie") == 0)
				{
					...
					if (strcmp(" UserID", strtok_r(NULL, "=", &strtokr_save)) == 0)
					{
		...
		else
		{
			if (MethodIsConditional(http_method))
			{
				...
				// Se la data che viene inserita da terminale (rappresenta il file locale sul client) è maggiore
				// rispetto a quella del file nel server, significa che il file presente sul server non è 
				// stato modificato
				//
				// st_mtime rappresenta la data di ultima modifica (convertita in secondi con tv_sec), e la
				// compariamo alla data inserita da terminale o da cache. 
