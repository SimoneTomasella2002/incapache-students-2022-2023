...
/*** TO BE DONE 6.0 START ***/
	...
	//DUBBIO la parte commentata, ma non dovrebbe servire dato che fà già tutto la funzione create_listening_socket
	/*
	if(bind(listen_fd, (struct sockaddr*) &www_root, sizeof(struct sockaddr_un)) < 0)
		fail_errno("Error: cannot bind address\n");

	if(listen(listen_fd, BACKLOG) < 0)
		fail_errno("Error: listen returned an error\n");
	*/
...
/*** TO BE DONE 6.0 START ***/
		...
		if(pthread_create(thread_ids[i], NULL, client_connection_thread, connection_no[i]) != 0)
			fail_errno("Error: cannot create pthread in run_webserver\n");
		*/
		pthread_create(&thread_ids[i], NULL, &client_connection_thread, &i);
