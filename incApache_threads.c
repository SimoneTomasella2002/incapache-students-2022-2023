...
/*** TO BE DONE 6.1 START ***/
	...
	// Vogliamo fare in modo di capire se un thread di connessione ha prima di lui un thread di risposta
	// 
	// In caso non ci siano, allora inseriamo in to_join[connection_idx] l'indirizzo del nuovo thread, mentre nel
	// nella to_join[new_thread_idx] inseriamo NULL (perchè sarebbe il primo thread di risposta)
	// 
	// Nel caso ci siano, allora dobbiamo salvare nella to_join[new_thread_idx] l'indirizzo del precedente thread
	// (salvato nella to_join[connection_idx]), e salviamo nella to_join[connection_idx] l'indirizzo del nuovo
	// thread di risposta
	//
	// Da vedere come una lista inversa, alla quale accediamo a partire dal thread di connessione
